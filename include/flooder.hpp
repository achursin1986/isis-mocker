...
class Flooder {
  public:
       ...
       Flooder(int id, std::unordered_map<std::string, std::string>& lsdb, IO& io , std::atomic_bool& state, bool update): lsdb_(lsdb), mocker_id_(id),io_(io), state_(state), holdtimer_(timer_,boost::posix_time::seconds(tick_interval_)),updatedb_(update){
                 timer_th_ = std::thread([&](){ holdtimer_.expires_from_now( boost::posix_time::seconds(tick_interval_)); holdtimer_.async_wait(boost::bind(&Flooder::startTimer,this)), timer_.run(); });
